...
train_set = train_datagen.flow_from_directory(
    train_path, target_size=target_image_size, batch_size=32, class_mode="categorical"
)

# Test data
test_set = test_datagen.flow_from_directory(
    test_path, target_size=target_image_size, batch_size=32, class_mode="categorical"
)


csv_logger = CSVLogger(
    filename="D:/ResearchPaperCode/csv/inception.csv", append=False
)


checkpoint = ModelCheckpoint(
    filepath="D:/Project/Models/inception.keras", verbose=2, save_best_only=True
)
...
# Save as a CSV file
report_df.to_csv("D:/ResearchPaperCode/ModelCodeFiles/classification/inception.csv", index=True)
# Confusion matrix
...
plt.savefig(os.path.join('D:/ResearchPaperCode/ModelCodeFiles/confusion', 'confusion_matrix_inception.png'))  # Save the confusion matrix
plt.close()
